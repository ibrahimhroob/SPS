...
#####################################################################
class TestModule(LightningDataModule):
    def __init__(self, cfg):
        ...
        test_scans, test_poses, test_map_tr = self.get_scans_poses(test_seqs, 100)
    ...
    def cash_scans(self, scans_pth, poses_pth, map_tr_pths):
        ...
        for scan_pth, pose_pth, map_tr_pth in tqdm(zip(scans_pth, poses_pth, map_tr_pths), total=len(scans_pth)):
            # Load scan and poses:
            scan_data = np.load(scan_pth) if scan_pth[:-3] == 'npy' else np.loadtxt(scan_pth)
    ...
    def get_scans_poses(self, seqs, length):
        seq_scans = []
        seq_poses = []
