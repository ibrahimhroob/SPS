...
class BacchusDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg['DATA']['ME_PRUNE']:
            submap_points = self.prune_map_points(scan_points, self.map[:, :3])
        else: 
    ...
    def prune_map_points(self, scan, pc_map):
        ...
        quantization = torch.Tensor([ds, ds, ds]) #.to(self.device)
        ...
        elapsed_time = time.time() - start_time
        # print('Minkowski Pruning time: %.3f s' % elapsed_time)

        return  submap_points.cpu()
...
if __name__ == "__main__":
    ...
    for batch in tqdm(train_dataloader):
        ...
        for b in batch_indices:
            ...
            # np.savetxt(os.path.join(debug_dir, 'scan.asc'), torch.hstack(scan_points, scan_labels).numpy())
            # np.savetxt(os.path.join(debug_dir, 'map.asc'), torch.hstack(map_points, map_labels).numpy())
